...
public class QuestionDaoImpl extends BaseDaoImpl<Question, String> implements QuestionDao {
	...
	private void saveQuestion(Question entity, Session session) {
		...
		}else if(quType==QuType.DEGREEFEEDBACK){
			saveDegreeFeedback(entity, session);
	 ...
	 * @param session
	 */
	private void saveDegreeFeedback(Question entity, Session session ){
		...
		for (DegreeFeedbackItem degreeFeedbackItem: degreeFeedbackItems){
			degreeFeedbackItem.setQuId(entity.getId());
			session.saveOrUpdate(degreeFeedbackItem);
			List<DegreeFeedbackItemItem> itemItems = degreeFeedbackItem.getItemList();
			for(DegreeFeedbackItemItem itemItem: itemItems){
				itemItem.setItemId(degreeFeedbackItem.getId());
				session.saveOrUpdate(itemItem);
			}
			session.saveOrUpdate(degreeFeedbackItem);
		}
	}
