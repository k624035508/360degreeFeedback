	...
	public void getQuestionOption(Question question) {
		...
		if(quType==QuType.RADIO || quType==QuType.COMPRADIO){
		 ...
		 question.setQuScores(quScores);
			...
			List<DegreeFeedbackItem> feedbacks = feedbackItemManager.findByQuId(quId);
			for(DegreeFeedbackItem degreeFeedbackItem: feedbacks){
				List<DegreeFeedbackItemItem> itemItems = feedbackItemItemManager.findByItemId(degreeFeedbackItem.getId());
				degreeFeedbackItem.setItemList(itemItems);
			};
			question.setDegreeFeedbackItems(feedbacks);
