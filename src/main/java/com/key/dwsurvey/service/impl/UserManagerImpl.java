...
//				entity.setSalt(salt);
	...
	public Page<User> findPage(Page<User> page, User entity) {
		...
		criterions.add(Restrictions.ne("id", "1"));
	...
	public List<User> findUserListByDepartment(String departmentId){
		...
		List<Criterion> criterions=new ArrayList<Criterion>();
		criterions.add(Restrictions.eq("department", departmentId));
		criterions.add(Restrictions.eq("isLeader", 0));
		page = userDao.findPageList(page, criterions);
	...
	public void resetUserDepartment(String departmentId) {
		userDao.resetUserDepartment(departmentId);
	}

	public User findByUserName(String userName){
		Criterion criterion=Restrictions.eq("name", userName);
		return userDao.findFirst(criterion);
	}
}
