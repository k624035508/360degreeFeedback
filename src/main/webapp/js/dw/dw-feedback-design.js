...
//						quLogicItem.find("input[name='logicEvent']").val(logicEvent);
    ...
    $("#dwDialogQuSetSave").click(function(){
        if(dwDialogObj!=null){
            ...
            if(quType=="RADIO" || quType=="CHECKBOX" || quType=="FILLBLANK"){
                ...
                //     }
                // });
                scoreNumTableTr.append("<td>1</td>");
            }else if(quType==="MULTIFILLBLANK"){
    ...
    $("#dwDialogSurveyAttrSave").click(function(){
        var ownWeight=parseInt($("input[name='ownWeight']").val());
        var topWeight=parseInt($("input[name='topWeight']").val());
        var middleWeight=parseInt($("input[name='middleWeight']").val());
        var bottomWeight=parseInt($("input[name='bottomWeight']").val());
        if(ownWeight + topWeight + middleWeight + bottomWeight != 100){
            alert("权重总和必须为100%");
            return false;
        }
        //调用保存事件
    ...
    function saveSurvey(callback){
        ...
        if( svyNmSaveTag==="0" || svyNoteSaveTag==="0" || svyAttrSaveTag==="0"){
            var url=ctx+"/design/my-feedback-design!ajaxSave.action";
            var data="feedbackId="+questionBelongId;
            if(svyNmSaveTag==="0"){
            ...
            var ownWeight=$("input[name='ownWeight']").val();
...
//				quBody.hide("slow",function(){$(this).remove();resetQuItem();});
    ...
    $(".addOption,.addColumnOption,.addRowOption").click(function(){
        ...
        if(quType=="RADIO"){
            ...
            editAble(addScoreItem(quItemBody, "新选项1"));
        }else if(quType=="DEGREEFEEDBACK"){
            editAble(addFeedbackItem(quItemBody, "新指标"));
        }else if(quType=="ORDERQU"){
    ...
    //指标考核添加行选项
    $(".addOptionItem").unbind();
    $(".addOptionItem").click(function(){
        var quItemBody=$(this).parents(".surveyQuItemBody");
        var index = $(".addOptionItem").index(this);
        console.log(index);
        editAble(addFeedbackItemItem(quItemBody, "新评分项1", index));
    })
    ...
    $(".dwOptionUp").click(function(){
        ...
        if(hv==3){
            ...
            if(quType==="RADIO" || quType==="CHECKBOX" || quType==="ORDERQU"){
                ...
                nextLi=$(curEditObj).parents("tr.feedbackTableTr");
                prevLi=nextLi.prev(".feedbackTableTr");
                var prevLiHtml=prevLi.html();
                nextLiAfterHtml="<tr class='feedbackTableTr'>" + prevLiHtml + "</tr>";
            }else if(quType==="MULTIFILLBLANK"){
            ...
            if(nextLi!=null){
                if(prevLi[0]){
                    ...
                    $(nextLi).find("input[name='quOptionDescSaveTag']").val(0);
                    $(nextLi).next().find("input[name='quOptionDescSaveTag']").val(0);
                    var quItemBody=$(curEditObj).parents(".surveyQuItemBody");
                    quItemBody.find("input[name='saveTag']").val(0);
                }else{
                    alert("已经是第一个了！");
    ...
    function dwOptionUp(prevTd,nextTd){
        ...
        $(nextTd).find("input[name='quOptionDescSaveTag']").val(0);
        $(nextTd).next().find("input[name='quOptionDescSaveTag']").val(0);
        var quItemBody=$(curEditObj).parents(".surveyQuItemBody");
        quItemBody.find("input[name='saveTag']").val(0);
    }
    ...
    function dwOptionUp_1(prevTr,nextTr){
        ...
        $(nextTd).find("input[name='quItemSaveTag']").val(0);
        $(prevTd).find("input[name='quOptionDescSaveTag']").val(0);
        $(nextTd).find("input[name='quOptionDescSaveTag']").val(0);
        var quItemBody=$(curEditObj).parents(".surveyQuItemBody");
        quItemBody.find("input[name='saveTag']").val(0);
    }
    ...
    $(".dwOptionDown").click(function(){
        ...
        if(hv==3){
            ...
            if(quType==="RADIO" || quType==="CHECKBOX" || quType==="ORDERQU"){
                ...
                prevLi=$(curEditObj).parents("tr.feedbackTableTr");
                nextLi=prevLi.next();
                var nextLiHtml=nextLi.html();
                prevLiBeforeHtml="<tr class='feedbackTableTr'>"+nextLiHtml+"</tr>";
            }else if(quType==="MULTIFILLBLANK"){
            ...
            if(nextLi[0]){
                ...
                $(prevLi).find("input[name='quOptionDescSaveTag']").val(0);
                $(prevLi).prev().find("input[name='quOptionDescSaveTag']").val(0);
                var quItemBody=$(curEditObj).parents(".surveyQuItemBody");
                quItemBody.find("input[name='saveTag']").val(0);
            }else{
                alert("已经是最后一个了！");
    ...
    function dwOptionDown(prevTd,nextTd){
        ...
        $(prevTd).find("input[name='quOptionDescSaveTag']").val(0);
        $(prevTd).next().find("input[name='quOptionDescSaveTag']").val(0);
        var quItemBody=$(curEditObj).parents(".surveyQuItemBody");
        quItemBody.find("input[name='saveTag']").val(0);
    }
    ...
    //  评测考核项目
    $(".dwOptionItemUp").unbind();
    $(".dwOptionItemUp").click(function(){
        var quItemBody=$(curEditObj).parents(".surveyQuItemBody");
        var nextLi=$(curEditObj).parents("li.quItemItemEditLi");
        var prevLi=nextLi.prev();
        var prevLiHtml = prevLi.html();
        var nextLiAfterHtml="<li class='quItemItemEditLi'>" + prevLiHtml + "</li>";
        if(nextLi!=null){
            if(prevLi[0]){
                $(nextLi).after(nextLiAfterHtml);
                prevLi.hide();
                ...
                bindQuHoverItem();
                $(curEditObj).click();
                $(nextLi).find("input[name='quItemItemSaveTag']").val(0);
                $(nextLi).next().find("input[name='quItemItemSaveTag']").val(0);
                var quItemBody=$(curEditObj).parents(".surveyQuItemBody");
                quItemBody.find("input[name='saveTag']").val(0);
            } else {
                alert("已经是第一个");
    ...
    $(".dwOptionItemDown").unbind();
    $(".dwOptionItemDown").click(function(){
        var quItemBody=$(curEditObj).parents(".surveyQuItemBody");
        var prevLi=$(curEditObj).parents("li.quItemItemEditLi");
        var nextLi=prevLi.next();
        var nextLiHtml=nextLi.html();
        var prevLiBeforeHtml="<li class='quItemItemEditLi'>" + nextLiHtml + "</li>";
        if(nextLi[0]){
            $(prevLi).before(prevLiBeforeHtml);
            nextLi.hide();
            nextLi.remove();
            ...
            bindQuHoverItem();
            $(curEditObj).click();
            $(prevLi).find("input[name='quItemItemSaveTag']").val(0);
            $(prevLi).prev().find("input[name='quItemItemSaveTag']").val(0);
            var quItemBody=$(curEditObj).parents(".surveyQuItemBody");
            quItemBody.find("input[name='saveTag']").val(0);
        } else {
            alert("已经是最后一个");
    ...
    $(".dwOptionItemDel").unbind();
    $(".dwOptionItemDel").click(function () {
        var optionParent=null;
        optionParent=$(curEditObj).parents("li.quItemItemEditLi");

        var quOptionItemId=$(optionParent).find("input[name='quItemItemId']").val();
        if(quOptionItemId!=null && quOptionItemId!="0"){
            var url = ctx + "/design/feedback-item!ajaxDeleteItem.action";
            var data = "quItemItemId=" + quOptionItemId;
            $.ajax({
                url: url,
                data: data,
                type: "post",
...
function setSaveTag0(){
    ...
    }else if(thClass.indexOf("quOptionDescEdit")>0){
        //题目说明
        $(curEditObj).parents(".feedbackTableTr").find("input[name='quOptionDescSaveTag']").val(0);
    }else if(thClass.indexOf("quItemItemEdit")>0){
        //题目指标选项
        $(curEditObj).parent().find("input[name='quItemItemSaveTag']").val(0);
    }
...
function editAble(editAbleObj){
    ...
    if(thClass.indexOf("quCoTitleEdit")>0){
        ...
        //题目选项
        $("#dwCommonEditRoot ul.dwComEditOptionItemUl").hide();
        $("#dwCommonEditRoot ul.dwComEditOptionUl").show();
        $("#dwCommonEditRoot").addClass("quOptionEdit");
    }else if(thClass.indexOf("quOptionDescEdit")>0){
        //题目选项描述
        $("#dwCommonEditRoot ul.dwComEditOptionItemUl").hide();
        $("#dwCommonEditRoot ul.dwComEditOptionUl").hide();
        $("#dwCommonEditRoot").addClass("quDescEdit");
    }else if(thClass.indexOf("quItemItemEdit")>0){
        //考核评分项目的选项名称
...
function saveQus(quItemBody,callback){
    if(quItemBody[0]){
        ...
        if(quType=="RADIO"){
            ...
            saveFeedback(quItemBody, callback);
        }else if(quType=="ORDERQU"){
...
//*****指标选项****//
/* 保存指标 */
function saveFeedback(quItemBody,callback) {
    var saveTag=quItemBody.find("input[name='saveTag']").val();
    if(saveTag == 0){
        var url = ctx + "/design/feedback-item!ajaxSave.action";
        var quType=quItemBody.find("input[name='quType']").val();
        var quId=quItemBody.find("input[name='quId']").val();
        var orderById=quItemBody.find("input[name='orderById']").val();
        var isRequired=quItemBody.find("input[name='isRequired']").val();
        ...
        $.each(quItemOptions,function(i){
            ...
            $.each(quItemItemOptions,function(j){
                var optionItemValue=$(this).find("label.quItemItemEdit").html();
                var optionItemDescValue=quItemItemDescOptions.eq(j).find("label.quItemItemDescEdit").html();
 ...
 ** 添加指标选项
 **/
function addFeedbackItem(quItemBody, itemText){
    //得判断是否是table类型
    var newEditObj = null;
    //ul li处理
    var feedbackItemHtml = $("#feedbackTableModel").html();
    var quCoItemTable=quItemBody.find("table.feedbackTable");
    quCoItemTable.append("<tr class='feedbackTableTr'>"+feedbackItemHtml+"</tr>");
    quItemBody.find("input[name='saveTag']").val(0);


    newEditObj=quCoItemTable.find("tr.feedbackTableTr:last .quCoOptionEdit");
    //
    // newEditObj.text(itemText);
    // if(itemText==""){
         newEditObj.css("display","inline");
...
//添加指标评分项目
function addFeedbackItemItem(quItemBody, itemText, index){
    var newEditObj = null;
    var feedbackItemItemHtml = $("#feedbackTableItemModel").html();
    var feedbackItemItemDescHtml = $("#feedbackTableItemDescModel").html();
    var quCoItemItemTable = quItemBody.find("ul.quItemItemEditUl").eq(index);
    var quCoItemItemDescTable = quItemBody.find("ul.quItemItemDescEditUl").eq(index);
    quCoItemItemTable.append("<li class='quItemItemEditLi'>"+feedbackItemItemHtml+"</li>");
    quCoItemItemDescTable.append("<li class='quItemItemDescEditLi'>"+feedbackItemItemDescHtml+"</li>");
    quItemBody.find("input[name='saveTag']").val(0);

    newEditObj = quCoItemItemTable.find("li.quItemItemEditLi:last .editAble");
...
//删除指标
function deleteFeedbackOption() {
    var optionParent=null;
    optionParent=$(curEditObj).parents("tr.feedbackTableTr");

    var quOptionId=$(optionParent).find("input[name='quItemId']").val();
    if(quOptionId!=null && quOptionId!="0"){
        var url = ctx + "/design/feedback-item!ajaxDelete.action";
        var data = "quItemId=" + quOptionId;
        $.ajax({
            url: url,
            data: data,
